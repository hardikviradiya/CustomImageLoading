...
internal object Utils {
  const val THREAD_PREFIX = "ImageLoading-"
  const val THREAD_IDLE_NAME = THREAD_PREFIX + "Idle"
  private const val IMAGELOADER_CACHE = "imageLoader-cache"
  ...
  */
  ...
  fun createDefaultCacheDir(context: Context): File {
    val cache = File(context.applicationContext.cacheDir, IMAGELOADER_CACHE)
    if (!cache.exists()) {
      cache.mkdirs()
    }
    return cache
  ...
  fun isAirplaneModeOn(context: Context): Boolean {
    return try {
      ...
      // some devices might crash here, assume that
