...
import com.hardik.customimageloading.imageLoading.Utils.OWNER_HUNTER
import com.hardik.customimageloading.imageLoading.Utils.THREAD_PREFIX
import com.hardik.customimageloading.imageLoading.Utils.VERB_DECODED
import com.hardik.customimageloading.imageLoading.Utils.VERB_EXECUTING
import com.hardik.customimageloading.imageLoading.Utils.VERB_JOINED
import com.hardik.customimageloading.imageLoading.Utils.VERB_REMOVED
import com.hardik.customimageloading.imageLoading.Utils.VERB_TRANSFORMED
...
internal open class BitmapHunter(
  ...
  var priority: ImageLoader.Priority = action.request.priority
  ...
  fun hunt(): Bitmap? {
    ...
    imageLoader.bitmapDecoded(bitmap)
    ...
    val transformedResult =
      applyTransformations(imageLoader, data, transformations, result) ?: return null
    val transformedBitmap = transformedResult.bitmap
    imageLoader.bitmapTransformed(transformedBitmap)
  ...
  private fun computeNewPriority(): ImageLoader.Priority {
    val hasMultiple = actions?.isNotEmpty() ?: false
    val hasAny = action != null || hasMultiple

    // Hunter has no requests, low priority.
    if (!hasAny) {
      return ImageLoader.Priority.LOW
    }

    var newPriority = action?.request?.priority ?: ImageLoader.Priority.LOW
  ...
  companion object {
    ...
    fun forRequest(
      ...
      val request = action.request
      val requestHandlers = imageLoader.requestHandlers

      // Index-based loop to avoid allocating an iterator.
      for (i in requestHandlers.indices) {
        val requestHandler = requestHandlers[i]
        if (requestHandler.canHandleRequest(request)) {
          return BitmapHunter(imageLoader, dispatcher, cache, action, requestHandler)
        }
      }

      return BitmapHunter(imageLoader, dispatcher, cache, action, ERRORING_HANDLER)
    }
